<template>
    <div>
        ...
        <attachments  :attachments="question.attachments"  :playerObj="playerObj"  @playedObj="playedObj" :id="id"></attachments>
        <div v-if="moduleId==124">
        ...
        <div>
            <div v-if="isShowAnswer">
                <div class="u-question-item">
                    <span class="color-999999 inline-b">学生答案：</span>
                    <span v-if="moduleId == 15 && question.userAnswer && question.userAnswer.answers && question.userAnswer.answers[0] && question.userAnswer.answers[0].content">
                    <theAudio :src="question.userAnswer.answers[0].content" :id="id+10001" :playerObj="playerObj" @playedObj="playedObj"></theAudio>
                    </span>
                    <div class="inline-b" 
                        v-else-if="moduleId==124" >
                        <span v-if="padShow" class="editImg" v-html="matchUrl(question.userAnswer && question.userAnswer.answers && question.userAnswer.answers[0] && question.userAnswer.answers[0].content)"
                        onClick="brigeCanvas(this)">
                        </span> 
                       <div v-else class="fl" v-html="matchUrl(question.userAnswer && question.userAnswer.answers && question.userAnswer.answers[0] && question.userAnswer.answers[0].content)" @click="imgFun(question)"></div> 
                    </div>   
                    <bigImgShow :imgSrc="imgUrl" v-if="bigImgShow && imgFlag" @imgBigShow="imgBigShow"></bigImgShow>
                    <div class="u-shortAnswer">
                        <div class="assessIconBox f-error_txt" v-html="setAssessIcon(question.userAnswer)" @click.stop="setNewAns(question)"></div>
            ...
            <listenTabTopic v-if="moduleId==15" :infoAnswer="question.ref_info" :listenTextModule="question.textModule" :qtype="6"></listenTabTopic>
            <tabTopic :infoAnswer='question.ref_info && question.ref_info.answers[0]' :knowledgePoints="question.knowledge_points" :explains="question.explains" :tags="question.tags" :qtype="6"  v-if="moduleId==124"></tabTopic>
        </div>
    </div>
</template>
...
<script>
    ...
    import bigImgShow from './alertBigImg.vue'
    import theAudio from '../player/audio.vue'  // 音频组件
    import tabTopic from './tabTopic.vue' // 在线的tab 
    ...
    export default {
        ...
        methods:{
            ...
            matchUrl(val){
              let reg = /\S.[jpg|png|jpeg]$/
              if(reg.test(val)) {
                this.bigImgShow = true;
                this.imgUrl=val;
                return '<img src='+val+' class="width_20">'
              }else {
                return val;
              }
            },
            ...
            setNewAns(question){
                //过滤没有作答的 批改不显示
                if(question.userAnswer.assess==6 || question.is_objective==1){
                    return false;
                }
