<template>
    <div>
        <div>
            <p>{{question.question_description}}第1小题小题说明小题说明小题说明</p>
            <span v-if="question.sid>-1&&moduleId!=15">{{question.sid +1}}. </span> 
            <div class="inline" v-html="question.stem"> </div>
            <span v-if="question.question_ref_sorce">（{{question.question_ref_sorce || 0}}分）</span>
        </div>
        ...
        <div class="option-box clearfix">
            <div class="option-item" v-for="(option,opIndex) in question.options">
                ...
                <attachments  :playerObj="playerObj"  @playedObj="playedObj"   :id="question.id" :attachments="option.attachments"></attachments>
            </div>
        </div>
        ...
        <listenTabTopic v-if="moduleId==15" :infoAnswer="question.ref_info.answers" :listenTextModule="question.textModule" :qtype="1"></listenTabTopic>
        <tabTopic :infoAnswer='question.ref_info.answers' :knowledgePoints="question.knowledge_points" :explains="question.explains" :qtype="1" :tags="question.tags" v-if="moduleId==124"></tabTopic>
    </div>
</template>
